...
import prototype2.attachment.generic.AttackAttachment;

public class SageAttackAttachment extends Attachment {
    AttackAttachment attack;

    public SageAttackAttachment(Robot robot) {
        super(robot);
    ...
    public void decideAttack() throws GameActionException {
        ...
        for (int i = 0; i < nearbyEnemyRobots.length; i++) {
            ...
            if (rob.type.isBuilding() && rob.mode == RobotMode.TURRET) {
                ...
                chargeDamage += (int) (rob.type.getMaxHealth(rob.level) * 0.22);
            }
        }
        if (furyDamage > 45) {
