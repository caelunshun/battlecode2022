package JFPROTO.archon;

import battlecode.common.*;
import JFPROTO.Attachment;
import JFPROTO.BotConstants;
import JFPROTO.Robot;
import JFPROTO.Util;
import JFPROTO.build.BuildType;
import JFPROTO.build.BuildWeightTable;
import JFPROTO.comms.BecomeSwarmLeader;
import JFPROTO.generic.SymmetryType;
    ...
    private void incrementBuildWeights() throws GameActionException {
        ...
        if (rc.getRoundNum() < 60) {
            if ((rc.getMapHeight() * rc.getMapWidth()) < 1800){
                if(noLeadCollected) {
                    buildWeights.addWeight(JFPROTO.build.BuildType.MINER, 40);
                    buildWeights.addWeight(JFPROTO.build.BuildType.SOLDIER, 15);
                } else {
                    buildWeights.addWeight(JFPROTO.build.BuildType.MINER, 25);
                    buildWeights.addWeight(JFPROTO.build.BuildType.SOLDIER, 30);
                }
            } else {
                if(noLeadCollected){
                    buildWeights.addWeight(JFPROTO.build.BuildType.MINER, 30);
                    buildWeights.addWeight(JFPROTO.build.BuildType.SOLDIER, 15);
                } else {
                    buildWeights.addWeight(JFPROTO.build.BuildType.MINER, 40);
                    buildWeights.addWeight(JFPROTO.build.BuildType.SOLDIER, 25);
                }
            ...
            if(noLeadCollected){
                ...
                buildWeights.addWeight(JFPROTO.build.BuildType.MINER, 5);
            }
        }
