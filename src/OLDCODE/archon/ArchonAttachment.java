package OLDCODE.archon;

import battlecode.common.*;
import OLDCODE.Attachment;
import OLDCODE.BotConstants;
import OLDCODE.Robot;
import OLDCODE.Util;
import OLDCODE.build.BuildType;
import OLDCODE.build.BuildWeightTable;
import OLDCODE.comms.BecomeSwarmLeader;
import OLDCODE.generic.SymmetryType;
    ...
    private void incrementBuildWeights() throws GameActionException {
        ...
        if (rc.getRoundNum() < 60) {
            if ((rc.getMapHeight() * rc.getMapWidth()) < 1800){
                if(noLeadCollected) {
                    buildWeights.addWeight(OLDCODE.build.BuildType.MINER, 40);
                    buildWeights.addWeight(OLDCODE.build.BuildType.SOLDIER, 15);
                } else {
                    buildWeights.addWeight(OLDCODE.build.BuildType.MINER, 25);
                    buildWeights.addWeight(OLDCODE.build.BuildType.SOLDIER, 30);
                }
            } else {
                if(noLeadCollected){
                    buildWeights.addWeight(OLDCODE.build.BuildType.MINER, 30);
                    buildWeights.addWeight(OLDCODE.build.BuildType.SOLDIER, 15);
                } else {
                    buildWeights.addWeight(OLDCODE.build.BuildType.MINER, 60);
                }
            ...
            if(noLeadCollected){
                ...
                buildWeights.addWeight(OLDCODE.build.BuildType.MINER, 5);
            }
        }
